 ...
 * @Last Modified by: lk
 * @Last Modified time: 2020-01-03 13:47:57
 */
 ...
 * @param {对象} source
 */
export function validateInputs(source) { // NOSONAR
  if (!source && typeof source !== 'object') {
    throw new Error('error arguments', 'shallowClone')
  }
  const targetObj = source.constructor === Array ? [] : {} // NOSONAR
  validateInputsEach(source, targetObj)
  return targetObj
}

function validateInputsEach(source, targetObj) {
  Object.keys(source).forEach((keys) => {
    if (source[keys] && typeof source[keys] === 'object') {
